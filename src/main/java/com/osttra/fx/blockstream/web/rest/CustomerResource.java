...
public class CustomerResource {
    ...
    public CustomerResource(UserService userService, CustomerRepository customerRepository) {
    ...
    private boolean onlyContainsAllowedProperties(Pageable pageable) {
        return pageable.getSort().stream().map(Sort.Order::getProperty).allMatch(UserResource.ALLOWED_ORDERED_PROPERTIES::contains);
    }
    ...
    public ResponseEntity<AdminUserDTO> getCustomer(@PathVariable String id) {
        log.debug("REST request to get Customer : {}", id);
        //        Optional<Customer> customer = customerRepository.findById(id);
        //        ExampleMatcher customExampleMatcher = ExampleMatcher.matchingAny()
        //            .withMatcher("customerLegalEntity", ExampleMatcher.GenericPropertyMatchers.contains().ignoreCase());
        //        Optional<Customer> customer = customerRepository.findOne(Example.of(new Customer().customerLegalEntity(id), customExampleMatcher));
        //        return ResponseUtil.wrapOrNotFound(customer);
        return ResponseUtil.wrapOrNotFound(userService.getUserWithAuthoritiesByLogin(id).map(AdminUserDTO::new));
    }
